...
import { FiChevronRight, FiEdit3, FiInfo, FiLogOut, FiMoon, FiSun, FiUser } from "react-icons/fi";
...
export default function Settings() {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-teal-50 via-white to-cyan-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 p-4 pb-24 sm:p-6">
            <div className="mx-auto max-w-2xl space-y-4">
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
                    ...
                    <button
                        ...
                        className="flex w-full items-center gap-4 p-4 transition hover:bg-slate-50 dark:hover:bg-slate-800 active:bg-slate-100 dark:active:bg-slate-700"
                    >
                        ...
                        <div className="flex-1 min-w-0 text-left">
                            ...
                            <p className="truncate text-xs text-slate-400 dark:text-slate-500">{authUser?.email}</p>
                        </div>
                        <div className="flex items-center gap-1 text-xs font-medium text-teal-600 dark:text-teal-400">
                            Edit
                            <FiChevronRight size={14} className="text-slate-300 dark:text-slate-600" />
                        </div>
                    </button>
                </div>
                    ...
                    <div className="p-4">
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <button
                                ...
                                aria-checked={isDark}
                                onClick={toggleTheme}
                                className={`relative ml-2 inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full border-2 transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-900 ${
                                    isDark ? "border-teal-500 bg-teal-500" : "border-slate-200 bg-slate-200"
                                }`}
                            >
                                <span className={`inline-block h-4 w-4 rounded-full bg-white shadow-sm transition-transform duration-200 ${isDark ? "translate-x-5" : "translate-x-0.5"}`} />
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
                    <div className="flex items-center gap-2.5 border-b border-slate-100 dark:border-slate-700/50 bg-gradient-to-r from-slate-50 to-teal-50/50 dark:from-slate-800 dark:to-slate-800 px-4 py-3">
                        ...
                        <p className="text-sm font-bold text-slate-800 dark:text-slate-100">Community Contributions</p>
                    </div>

                    <div className="p-4">
                        <div className="flex items-center justify-between gap-4">
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Volunteer Mode</p>
                                <p className="mt-0.5 text-xs leading-relaxed text-slate-500 dark:text-slate-400">
                            ...
                            >
                                <span className={`inline-block h-4 w-4 rounded-full bg-white shadow-sm transition-transform duration-200 ${isVolunteerEnabled ? "translate-x-5" : "translate-x-0.5"}`} />
                            </button>
                        </div>

                        <div className={`mt-3 overflow-hidden transition-all duration-300 ${isVolunteerEnabled ? "max-h-20 opacity-100" : "max-h-0 opacity-0"}`}>
                            <div className="flex items-center gap-2 rounded-xl border border-teal-200 dark:border-teal-700/50 bg-teal-50 dark:bg-teal-900/20 px-3 py-2.5">
                ...
                <div className="flex gap-3 rounded-2xl border border-amber-200 dark:border-amber-700/40 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 px-4 py-3.5 shadow-sm">
                    ...
                    <div>
                        <p className="text-xs font-bold uppercase tracking-wide text-amber-800 dark:text-amber-300">Community Guidelines</p>
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
                    <button
                        ...
                        className="flex w-full items-center gap-3 px-4 py-4 text-left transition hover:bg-rose-50 dark:hover:bg-rose-900/20 active:bg-rose-100 dark:active:bg-rose-900/30"
                    >
                        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-rose-50 dark:bg-rose-900/30 text-rose-500">
                            <FiLogOut size={16} />
                        </span>
                        <div className="flex-1">
                            <p className="text-sm font-semibold text-rose-600 dark:text-rose-400">Sign Out</p>
                            <p className="text-xs text-slate-400 dark:text-slate-500">You'll need to log in again to access the app</p>
                        </div>
                    </button>
                </div>
