...
export default function Home() {
    ...
    const stats = [
        ...
        { label: "Mosques Verified",  value: 9,  trend: "90% accuracy",  icon: FiCheckCircle,color: "from-emerald-500 to-green-600", bg: "bg-emerald-50", text: "text-emerald-700" },
    ];

    const recentActivity = [
        { icon: FiEdit3,       color: "bg-teal-100 text-teal-600",   title: "Masjid Noor — Fajr",        detail: "Updated congregation to 05:45 AM",        time: "2 hours ago"  },
    ...
    const suggestions = [
        { icon: FiClock,      title: "Verify Friday Jumu'ah timing",  note: "Multiple users have flagged outdated Friday schedules in your area." },
        { icon: FiEdit3,      title: "Confirm Ramadan timetable",     note: "Seasonal prayer times shift significantly — your local update would help many." },
        ...
        <div className="min-h-screen bg-gradient-to-b from-teal-50 via-white to-cyan-50 p-4 pb-28 sm:p-5">
            <div className="mx-auto max-w-2xl space-y-4">
                ...
                <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-teal-700 via-cyan-700 to-sky-700 p-5 text-white shadow-[0_20px_45px_-20px_rgba(14,116,144,0.75)] sm:p-6">
                    ...
                    <div className="relative flex items-start justify-between gap-4">
                        <div className="flex-1">
                            <p className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1 text-[11px] font-semibold uppercase tracking-widest ring-1 ring-white/20">
                                <FiActivity size={10} />
                                Community Dashboard
                            </p>
                            <h1 className="mt-3 text-2xl font-extrabold leading-tight sm:text-3xl">
                                Assalamu Alaikum, <br className="sm:hidden" />{displayName} 👋
                            </h1>
                            <p className="mt-1.5 text-sm text-cyan-100">
                                Thank you for keeping local mosque prayer data accurate and up to date.
                            </p>
                        ...
                        {/* avatar */}
                        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white/20 text-base font-bold ring-2 ring-white/30">
                            {initials}
                        </div>
                    </div>
                    ...
                    <Link
                        to="/app"
                        className="mt-5 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-teal-800 shadow-lg transition hover:bg-teal-50 active:scale-95"
                    >
                        <FiMapPin size={15} />
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-teal-50 text-teal-600 transition group-hover:bg-teal-100">
                            <FiMapPin size={18} />
                        </span>
                        <div>
                            <p className="text-sm font-bold text-slate-800">Mosque Map</p>
                            <p className="text-[11px] text-slate-400">Find & explore</p>
                        </div>
                    </Link>
                    <Link
                        to="/settings"
                        className="group flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-cyan-200 hover:shadow-md"
                    >
                        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-cyan-50 text-cyan-600 transition group-hover:bg-cyan-100">
                            <FiSettings size={18} />
                        </span>
                        <div>
                            <p className="text-sm font-bold text-slate-800">Settings</p>
                            <p className="text-[11px] text-slate-400">Preferences</p>
                        </div>
                    </Link>
                </div>
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div className="grid grid-cols-3 divide-x divide-slate-100">
                        {stats.map((s) => (
                            <div key={s.label} className="flex flex-col items-center gap-1 px-3 py-4 text-center">
                                <span className={`flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-br ${s.color} text-white shadow-sm`}>
                                    <s.icon size={14} />
                                </span>
                    ...
                    <div className="divide-y divide-slate-50">
                        {recentActivity.map((item) => (
                            <div key={item.title} className="flex items-start gap-3 px-4 py-3.5">
                                <span className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${item.color}`}>
                                    <item.icon size={14} />
                                </span>
                    ...
                    <div className="divide-y divide-slate-50">
                        {suggestions.map((item) => (
                            <div key={item.title} className="flex items-start gap-3 px-4 py-3.5">
                                ...
                                <FiChevronRight size={16} className="mt-1 shrink-0 text-slate-300" />
                            </div>
                        ))}
                    </div>
                </div>
