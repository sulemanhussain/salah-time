...
const FIELD_CLASS =
    "h-11 w-full rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 px-3.5 text-sm text-slate-800 dark:text-slate-100 outline-none transition placeholder:text-slate-300 dark:placeholder:text-slate-600 focus:border-teal-400 focus:bg-white dark:focus:bg-slate-800 focus:ring-2 focus:ring-teal-100 dark:focus:ring-teal-900/50";
...
export default function ProfileModal({ isOpen, onClose }: Props) {
    ...
    return createPortal(
        <Modal isOpen={isOpen} fullPage>
            <div className="relative flex flex-col flex-1 overflow-x-hidden bg-gradient-to-b from-teal-50 via-white to-cyan-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
                    <div className="flex-1 overflow-y-auto">
                        ...
                        <div className="top-0 z-20 relative overflow-hidden bg-gradient-to-r from-teal-700 via-cyan-700 to-sky-700 px-5 pb-6 pt-6 text-white shadow-[0_16px_40px_-20px_rgba(14,116,144,0.6)] sm:px-6">
                            ...
                            <div className="relative">
                                ...
                                <h3 className="mt-2.5 text-2xl font-extrabold leading-tight tracking-tight">Your Profile</h3>
                                <p className="mt-1 text-sm text-cyan-100">Keep your personal details accurate and up to date.</p>
                        ...
                        <div className="mx-auto max-w-3xl space-y-4">
                            ...
                            <Field label="Full Name" icon={FiUser}>
                                <input type="text" value={form.fullName} onChange={(e) => handleChange("fullName", e.target.value)} placeholder="e.g. Abdullah Hassan" className={FIELD_CLASS} />
                            </Field>
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`flex flex-col items-center gap-1.5 rounded-xl border px-2 py-3 text-xs font-semibold transition-all duration-150 ${
                                                    active
                                                        ? "border-teal-400 bg-teal-50 dark:bg-teal-900/30 text-teal-800 dark:text-teal-300 ring-1 ring-teal-300 shadow-sm"
                                                        : "border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:border-teal-200 hover:bg-teal-50/40 dark:hover:bg-teal-900/20"
                                                }`}
                                            >
                                                <opt.Icon size={28} />
                            ...
                            <Field label="Email Address" icon={FiMail}>
                                <input type="email" value={form.email} onChange={(e) => handleChange("email", e.target.value)} placeholder="you@example.com" className={FIELD_CLASS} />
                            </Field>

                            <Field label="Phone Number" icon={FiPhone}>
                                <input type="tel" value={form.phone} onChange={(e) => handleChange("phone", e.target.value)} placeholder="e.g. +44 7700 900000" className={FIELD_CLASS} />
                            </Field>

                            <Field label="City / Area" icon={FiMapPin}>
                                <input type="text" value={form.city} onChange={(e) => handleChange("city", e.target.value)} placeholder="e.g. Birmingham, UK" className={FIELD_CLASS} />
                            </Field>
                            ...
                            <Field label="About / Bio" icon={FiUser}>
                                <textarea
                                    ...
                                    rows={4}
                                    maxLength={300}
                                    className="w-full resize-none rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 px-3.5 py-3 text-sm text-slate-800 dark:text-slate-100 outline-none transition placeholder:text-slate-300 dark:placeholder:text-slate-600 focus:border-teal-400 focus:bg-white dark:focus:bg-slate-800 focus:ring-2 focus:ring-teal-100 dark:focus:ring-teal-900/50"
                                />
                                <p className="mt-1 text-right text-[11px] text-slate-400 dark:text-slate-500">{form.bio.length} / 300</p>
                            </Field>
                    ...
                    {/* footer */}
                    <div className="border-t border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-slate-900/95 px-4 py-3 backdrop-blur sm:px-6">
...
function Field({ label, icon: Icon, children }: { label: string; icon: React.ElementType; children: React.ReactNode }) {
    return (
        <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
            <div className="flex items-center gap-2.5 border-b border-slate-100 dark:border-slate-700/50 bg-gradient-to-r from-slate-50 to-teal-50/50 dark:from-slate-800 dark:to-slate-800 px-4 py-2.5">
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-teal-500 text-white">
                    <Icon size={10} />
                </span>
                <p className="text-xs font-bold uppercase tracking-wide text-slate-600 dark:text-slate-400">{label}</p>
            </div>
            <div className="p-3">
                {children}
            </div>
